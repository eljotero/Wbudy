...
tU16 calculateBrightness(tU16 channel0, tU16 channel1)
{
    ...
    tU16 finalResult;
    if ((float)(channel1 / channel0) > 0 && (float)(channel1 / channel0) <= 0.52)
    {
        finalResult = 0.0315 * channel0 - 0.0593 * channel0 * pow((channel1 / channel0), 1.4);
    }
    else if ((float)(channel1 / channel0) > 0.52 && (float)(channel1 / channel0) <= 0.65)
    {
        finalResult = 0.0229 * channel0 - 0.0291 * channel1;
    }
    else if ((float)(channel1 / channel0) > 0.65 && (float)(channel1 / channel0) <= 0.80)
    {
        finalResult = 0.0157 * channel0 - 0.0180 * channel1;
    }
    else if ((float)(channel1 / channel0) > 0.80 && (float)(channel1 / channel0) <= 1.3)
    {
        finalResult = 0.00388 * channel0 - 0.00260 * channel1;
    }
    else if ((float)(channel1 / channel0) > 1.3)
    {
        finalResult = 0;
    }
    return finalResult;
...
tU16 measureBrightness()
{
    ...
    command[1] = ((0x09 << 4) | 0x03);  // Wrtie 0x03 (so power up) the device
    ...
    command[0] = ((0x09 << 4) | 0x00);  // Change target register to control register
    command[1] = ((0x09 << 4) | 0x00);  // Wrtie 0x00 (so power down) the device
    i2cWrite(tslWriteAddress, command, 2);
